		...
		_data_path(dp), _cam_views_amount(cva)
{
	...
	for (int v = 0; v < _cam_views_amount; ++v)
	{
		...
		cout<<full_path.str();
		cout<<General::fexists(full_path.str())<<endl;
		assert(
...
void VoxelReconstruction::run(int argc, char** argv)
{
	...
	{
		bool has_cam = false;
		if (!General::fexists(_cam_views[v]->getDataPath() + _cam_views[v]->getCamPropertiesFile()))
			has_cam = Camera::detExtrinsics(_cam_views[v]->getDataPath(), General::CheckerboadVideo, General::IntrinsicsFile,
					_cam_views[v]->getCamPropertiesFile());
		else
			has_cam = true;
