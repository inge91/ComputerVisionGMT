...
void Glut::reset()
{
	...
	glLoadIdentity();
	perspectiveGL(50, scene3d.getAspectRatio(), 1, 20000);
...
void Glut::keyboard(unsigned char key, int x, int y)
{
	...
	if (key_i == 0)
	{
		...
		{
			bool paused = _glut->getScene3d().isPaused();
		...
		{
			bool rotate = _glut->getScene3d().isRotate();
...
#endif
			bool arcball = _glut->getScene3d().isShowArcball();
			scene3d.setShowArcball(!arcball);
		}
		else if (key == 'v' || key == 'V')
		{
			bool volume = _glut->getScene3d().isShowVolume();
			scene3d.setShowVolume(!volume);
		}
		else if (key == 'g' || key == 'G')
		{
			bool floor = _glut->getScene3d().isShowGrdFlr();
			scene3d.setShowGrdFlr(!floor);
		}
		else if (key == 'c' || key == 'C')
		{
			bool cam = _glut->getScene3d().isShowCam();
...
#endif
			bool info = _glut->getScene3d().isShowInfo();
			scene3d.setShowInfo(!info);
		}
		else if (key == 'o' || key == 'O')
		{
			bool origin = _glut->getScene3d().isShowOrg();
